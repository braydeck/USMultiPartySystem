...
- **Per-state view**: `IRVSankey` reused verbatim (`rounds`, `irvWinner`), above a
  final-two line reading winner code, winner %, runner-up code, runner-up %, and
  margin in pp.
...
- Quota marker line at `data.quotaPct`.
...
- The "Eliminated" sub-block loses its "Eliminated during IRV" caption, which was
  never accurate for this stage; it becomes "Did not reach quota", ordered by
  first-choice share.

### `components/senate/SenateCondorcetView.tsx`

Unchanged. Wrapped in a small `SenateCondorcetCard` in `SenateTab.tsx` alongside
its heading so the two branches of the slot are symmetrical.
