...
const ASSUMPTIONS: {
  ...
  },
  {
    ...
    body: 'Same-party candidates share their party’s posterior score, so they occupy consecutive ranks, and within that block every voter lists the slate in one fixed order. That is the strong-discipline limit case: real parties control their transfers imperfectly and unevenly, and how well a party manages them is part of how it converts votes into seats. Slate size does respond to local strength, by different rules per chamber: the House sets it from the Droop quota, where a party expecting to win a seat runs one more candidate than that, capped at half the district’s seats plus one, with a single candidate everywhere it polls under one local quota; the Senate uses fixed shares of the state, 12% for three candidates, 5% for two, 1% for one. The fixed 28-candidate presidential field is hand-specified instead, nine parties at three candidates and Order & Opportunity at one. So mis-nomination is possible, but cross-party slates and factional tickets are not modeled at all.',
  },
  {
    ...
    <div className="space-y-4">
      ...
      <Card className="p-5 bg-muted">
        ...
        <div className="space-y-1 text-xs text-muted-foreground">
          ...
          <div><span className="font-medium text-muted-foreground">STV counting:</span> Droop quota, weighted inclusive Gregory surplus transfers</div>
